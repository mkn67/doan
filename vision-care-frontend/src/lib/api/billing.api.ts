...
export const billingApi = {
  ...
  createHoaDon: async (data: HoaDonRequestDTO): Promise<HoaDonResponseDTO> => {
    const response = await axiosClient.post<HoaDonResponseDTO>('/api/hoa-don', data);
  ...
  createHoaDonJson: async (data: TaoHoaDonJsonRequest): Promise<HoaDonResponseDTO> => {
    const response = await axiosClient.post<HoaDonResponseDTO>('/api/hoa-don/json', data);
    return response.data;
  },
  ...
  getDanhSachHoaDon: async (): Promise<HoaDonResponseDTO[]> => {
    ...
    const response = await axiosClient.get<HoaDonResponseDTO[]>('/api/hoa-don');
    return response.data;
  },

  // 4. Thanh toán
  thanhToan: async (data: ThanhToanRequestDTO): Promise<ThanhToanResponseDTO> => {
    const response = await axiosClient.post<ThanhToanResponseDTO>('/api/thanh-toan', data);
    return response.data;
  },
  ...
  getThongKeTongQuan: async (): Promise<ThongKeTongQuanDTO> => {
    const response = await axiosClient.get<ThongKeTongQuanDTO>('/api/report/tong-quan');
    return response.data;
  },
  ...
  getThongKeDoanhThuTheoNgay: async (tuNgay?: string, denNgay?: string): Promise<ThongKeDoanhThuTheoNgayDTO[]> => {
    const response = await axiosClient.get<ThongKeDoanhThuTheoNgayDTO[]>('/api/report/doanh-thu-theo-ngay', {
      params: {
        tuNgay,
        denNgay
      }
  ...
  getChiTietDoanhThu: async (tuNgay?: string, denNgay?: string): Promise<DoanhThuResponseDTO[]> => {
    const response = await axiosClient.get<DoanhThuResponseDTO[]>('/api/report/chi-tiet-doanh-thu', {
      params: {
        tuNgay,
        denNgay
      }
